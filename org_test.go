...
func TestUserdirOrg(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		expectedConfig = filepath.Join(originalHome, "AppData", "Roaming", org, app)
		expectedData = filepath.Join(originalHome, "AppData", "Roaming", org, app, "data")
		expectedCache = filepath.Join(originalHome, "AppData", "Local", org, app)
	} else {
		expectedConfig = filepath.Join(originalHome, ".config", org, app)
		expectedData = filepath.Join(originalHome, ".local", "share", org, app)
		expectedCache = filepath.Join(originalHome, ".cache", org, app)
	}
